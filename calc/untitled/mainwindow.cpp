...
connect(ui->pushButton_tan,SIGNAL(clicked()),this,SLOT(operation()));
connect(ui->pushButton_Ans,SIGNAL(clicked()),this,SLOT(operation()));
connect(ui->pushButton_procent,SIGNAL(clicked()),this,SLOT(operation()));

connect(ui->pushButton_phatorial,SIGNAL(clicked()),this,SLOT(operation()));

connect(ui->pushButton_sqrt,SIGNAL(clicked()),this,SLOT(operation()));
...
//Вывод на экранчик
    ...
    ui->status->setText(QString::number(-1*(ui->status->text()).toDouble(),'g',10));
        ...
        ui->status->setText(QString::number(tan((ui->status->text()).toDouble()),'g',10));
        }else if(button->text() == "sqrt"){
        ui->status->setText(QString::number(sqrt((ui->status->text()).toDouble()),'g',10));
        }else if(button->text() == "x!"){
        ui->status->setText(QString::number(fact((ui->status->text()).toDouble()),'g',10));
        }else if(button->text() == "Ans"){
        ui->status->setText(QString::number(eq01));
        }else if(button->text() == "%"){
        ui->status->setText(QString::number((ui->status->text()).toDouble()*0.01,'g',10));
        }
}
...
{
    QPushButton *button = static_cast<QPushButton*>(sender());
    ui->status->setText(QString::number(3.14));
